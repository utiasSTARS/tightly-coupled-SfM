'''
Use this for preprocessing KITTI odometry data for the odometry experiments
'''
...
parser.add_argument("--source_dir", type=str, default='/path/to/KITTI-odometry/')
parser.add_argument("--target_dir", type=str, default='/desired/path/to/KITTI-odometry-preprocessed')
...
for resolution in ['med_res']:
    ...
    for seq in sequences:    
        ...
        for cam_num in ['02','03']:
            seq_info = {}
            print(cam_num)
            seq_dir = os.path.join(target_dir, seq +'_{}'.format(cam_num))
            print(seq_dir)
            ...
            if cam_num == '02':
                ...
                with concurrent.futures.ProcessPoolExecutor() as executor: 
                    for filename, output in zip(data.cam2_files, executor.map(load_image, data.cam2_files)):
                        ...
                        new_filename = os.path.join(target_dir, filename.split(args.source_dir)[1]).replace('sequences/','').replace('/'+seq+'/','/'+seq+'_'+cam_num+'/').replace('.png','.jpg')
            ...
            if cam_num == '03':
                ...
                with concurrent.futures.ProcessPoolExecutor() as executor: 
                    for filename, output in zip(data.cam3_files, executor.map(load_image, data.cam3_files)):
                        img, zoomx, zoomy, orig_img_width, orig_img_height = output
                        new_filename = os.path.join(target_dir, filename.split(args.source_dir)[1]).replace('sequences/','').replace('image_3', 'image_2').replace('/'+seq+'/','/'+seq+'_'+cam_num+'/').replace('.png','.jpg')
                        imageio.imwrite(new_filename, img)
                        seq_info['intrinsics_left'][i,0] *= zoomx
                        seq_info['intrinsics_left'][i,1] *= zoomy
            ...
            with open('{}/sequence_data'.format(seq_dir) + '.pkl', 'wb') as f:
