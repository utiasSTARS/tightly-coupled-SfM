...
parser.add_argument("--source_dir", type=str, default='/path/to/KITTI/raw/') #path to full raw dataset
parser.add_argument("--target_dir", type=str, default='/desired/path/to/KITTI-eigen-preprocessed')
...
for resolution in ['med_res']:
    ...
    for seq in ['train', 'val']: #we don't preprocess the test data.
        seq_info = {}
        filenames = []
        ...
        for line in img_data: 
            ...
            if seq == 'eigen_benchmark' or seq == 'eigen':
                data = pykitti.raw(args.source_dir, date, drive)
                end_idx = len(data)-1
                
                ### get 3 frames for training/val, and only need a target frame for testing
                if int(img_num) == 0:
                    data = pykitti.raw(args.source_dir, date, drive, frames=range(int(img_num),int(img_num)+3))
                    target_idx = 0
                    source_idx = [1,2]
                
                elif int(img_num) == end_idx:
                    data = pykitti.raw(args.source_dir, date, drive, frames=range(int(img_num)-2,int(img_num)+1))
                    target_idx = 2
                    source_idx = [0,1]
        ...
        seq_info['target_idx'] = target_idx_list
        seq_info['source_idx_list'] = source_idx_list
