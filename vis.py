...
def plot_img_array(img_array, nrow=3, save_file = None, cmap=False):
    ...
    if cmap != False:
        ax.imshow(img_grid[0],cmap=cmap)
    else:
    ...
    if save_file!=None:
        ...
        plt.savefig(save_file, bbox_inches='tight', dpi=400)
...
def plotN(data, labels=None):
    ...
    for i in range(0,num_plots):
        for idx, d in enumerate(data):
            ax[i].plot(d[:,i],label=labels[idx], linewidth=2/(idx+1),zorder=num_plots-i)
