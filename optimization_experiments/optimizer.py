...
class DepthOptimizer():
    ...
    def compute_optimization_loss(self, opt_step_num, img_idx, target_img, target_disparity, fwd_data, inv_data):
        ...
        if self.options['plotting'] == True:
            if opt_step_num == 0 or opt_step_num == self.options['epochs']-1:
                print('test', opt_step_num)
                if self.options['diff_img_argmin']==True:
                    diff_imgs_plotting = torch.cat([fwd_data['diff_img']*fwd_data['valid_mask'], diff_img_min*valid_mask_min],0)
                    ...
                    for i in range(0, target_img.shape[0]):
                        ...
                        plot_img_array(plotting_imgs.cpu().detach(), nrow=3, save_file = '{}/{}_{}/combined_iter_{}_sample_{}.pdf'.format(self.options['results_dir'], self.seq, img_idx, opt_step_num,i))
                        torch.save(plotting_imgs, '{}/{}_{}/combined_iter_{}_sample_{}.pt'.format(self.options['results_dir'], self.seq, img_idx, opt_step_num,i))

                else:
                    diff_imgs_plotting = diff_imgs_masked
                    img_recs_plotting = torch.cat([fwd_data['img_rec'], target_img],0)
                    valid_masks_plotting = fwd_data['valid_mask']                  
                plot_img_array(img_recs_plotting.cpu().detach(), nrow=1, save_file = '{}/{}_{}/img_iter_{}.png'.format(self.options['results_dir'], self.seq, img_idx, opt_step_num))
                plot_img_array(diff_imgs_plotting.cpu().detach(), cmap='magma', nrow=1, save_file = '{}/{}_{}/error_iter_{}.png'.format(self.options['results_dir'], self.seq, img_idx, opt_step_num))
                plot_img_array(diff_imgs_inv_masked.cpu().detach(), cmap='magma', nrow=1, save_file = '{}/{}_{}/error_inv_iter_{}.png'.format(self.options['results_dir'], self.seq, img_idx, opt_step_num))
                plot_img_array(valid_masks_plotting.cpu().detach(), nrow=1, save_file = '{}/{}_{}/valid_masks_iter_{}.png'.format(self.options['results_dir'], self.seq, img_idx, opt_step_num))
                plot_img_array(inv_data['valid_mask'].cpu().detach(), nrow=1, save_file = '{}/{}_{}/valid_masks_inv_iter_{}.png'.format(self.options['results_dir'], self.seq, img_idx, opt_step_num))
                plot_img_array(fwd_data['weight_mask'].cpu().detach(), nrow=1, save_file = '{}/{}_{}/weight_masks_iter_{}.png'.format(self.options['results_dir'], self.seq, img_idx, opt_step_num))
                plot_img_array(inv_data['weight_mask'].cpu().detach(), nrow=1, save_file = '{}/{}_{}/weight_masks_inv_iter_{}.png'.format(self.options['results_dir'], self.seq, img_idx, opt_step_num))

                if self.options['automasking'] == True:
                    if self.options['diff_img_argmin']==True:
                        auto_mask_plotting = torch.cat([fwd_data['auto_mask'],auto_mask],0)
                    else:
                        auto_mask_plotting = fwd_data['auto_mask']
                    plot_img_array(auto_mask_plotting.cpu().detach(), nrow=1, save_file = '{}/{}_{}/auto_masks_iter_{}.png'.format(self.options['results_dir'], self.seq, img_idx, opt_step_num))
                    plot_img_array(inv_data['auto_mask'].cpu().detach(), nrow=1, save_file = '{}/{}_{}/auto_masks_inv_iter_{}.png'.format(self.options['results_dir'], self.seq, img_idx, opt_step_num))
    ...
    def optimize_window(self, img_idx, data):
        ...
        target_img, source_img_list, gt_lie_alg_list, vo_lie_alg_list, flow_imgs, intrinsics, \
            target_img_aug, source_img_aug_list, gt_lie_alg_aug_list, vo_lie_alg_aug_list, intrinsics_aug = process_sample_batch(data,self.config)
        ...
        for i in range(0,self.options['epochs']):
            ...
            poses, poses_inv, outputs = solve_pose_iteratively(self.config['iterations'], depths, pose_model, target_img, source_img_list, intrinsics, return_errors=True)
            ...
            if i == 0 or i == self.options['epochs']-1:
                if self.options['plotting'] == True:
                    plot_img_array(target_disparity.cpu().detach().clone()**1.5, nrow=1, save_file = '{}/{}_{}/target_depth_iter_{}.png'.format(self.options['results_dir'], self.seq, img_idx, i))
                    plot_img_array(source_disparities.cpu().detach().clone()**1.5, nrow=1, save_file = '{}/{}_{}/source_depth_iter_{}.png'.format(self.options['results_dir'], self.seq, img_idx, i))
            
            if i == self.options['epochs']-1 or i==0:
