...
if __name__=='__main__':
    ...
    model_dir = path_to_ws + 'results/kitti-eigen-4-iter'

    cam_height=1.65
    median_scaling=True #align scale of predicted depth with ground truth using median depths
    ...
    load_pred_disps = True
    
    output_path = '{}/paper_plots_and_data/saved_eigen_predictions/disps_{}_split.npy'.format(path_to_ws, benchmark)
    ...
    with torch.no_grad():
        if load_pred_disps == False:
            for k, data in enumerate(test_dset_loaders):
                ...
                disparities = depth_model(target_img)
            ...
            pred_disps = np.load(output_path)        
            print(pred_disps.shape)    
    ...
    for i in range(pred_disps.shape[0]):
        ...
        if median_scaling == True:
            ratio = np.median(gt_depth[gt_depth < 80]) / np.median(pred_depth[gt_depth < 80])
            ratios.append(ratio)
            pred_depth *= ratio
